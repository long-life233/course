...
https://space.bilibili.com/35723238/?spm_id_from=333.999.0.0
...
新建两个平面光
    ...
    然后选择下方的图像。。。选择素材的金丝雀.hdr后缀贴图。。。

    然后ctr+alt+0，切换相机视角，n，固定相机视角
    ...
    右边的相机物体属性，位置，旋转，鼠标放在上面，i键记录关键帧；在进度条的120处，改变相机的位置、旋转，i键记录；

    框选进度条，右键选择插值模式，匀速播放

    最后，渲染，导出图像，渲染成视频或者图像
```
